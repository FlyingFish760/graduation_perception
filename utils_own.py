...
def convert_prescan_gt_to_yolo_gt(prescan_gt: np.ndarray, img_width, img_height, numid_to_classid, gtsdb_format: bool) -> str:
    '''
    ...
    gtsdb_format: if set to True, then transfert the prescan ground truth foramt to that of GTSDB. Ground truths of GTSDB bounds only traffic sign plates, while the whole traffic 
    sign (including the sign pole and sign plate) are bounded in Prescan ground truth.
    '''
    ...
    # Convert the ground truth to GTSDB's bounding area
    if gtsdb_format:
        b = t + (b - t) * (0.6 / (0.6 + 1.6))
